...
async function initiatePayChanguPayment(params) {
    ...
    const operator = operators.find(op => op.name.toLowerCase().includes(providerLower) ||
...
async function verifyPayChanguPayment(chargeId) {
...
async function processAutoQueue() {
    try {
        ...
        for (const act of activities) {
            ...
            await pool.query(`UPDATE tokens
                 SET status = 'completed'
                 WHERE "activityId" = $1
                   AND status = 'in_use'
                   AND "usedAt" + ($2 || ' milliseconds')::interval < NOW()`, [act.id, SESSION_DURATION_MS]);
            // Count active occupancy (in_use + ready)
            const activeRows = await query(`SELECT COUNT(*) AS cnt FROM tokens
                 WHERE "activityId" = $1 AND (status = 'in_use' OR status = 'ready')`, [act.id]);
            const activeCount = Number(activeRows[0].cnt);
            const availableSpots = act.capacity - activeCount;
            if (availableSpots > 0) {
                const queueRows = await query(`SELECT id FROM tokens
                     WHERE "activityId" = $1 AND status = 'queue'
                     ORDER BY "createdAt" ASC
                     LIMIT $2`, [act.id, availableSpots]);
                if (queueRows.length > 0) {
                    const ids = queueRows.map(r => r.id);
                    await pool.query(`UPDATE tokens SET status = 'ready' WHERE id = ANY($1::int[])`, [ids]);
                }
            }
            // Refresh occupancy count
            const newActiveRows = await query(`SELECT COUNT(*) AS cnt FROM tokens
                 WHERE "activityId" = $1 AND (status = 'in_use' OR status = 'ready')`, [act.id]);
            await pool.query(`UPDATE activities SET "currentOccupancy" = $1 WHERE id = $2`, [Number(newActiveRows[0].cnt), act.id]);
        }
    }
    catch (err) {
...
app.get('/api/tokens/validate/:code', async (req, res) => {
    try {
        ...
        if (isExpired) {
            ...
            isValid = true;
            statusMessage = 'Token is valid and ready for use';
            await pool.query(`UPDATE tokens SET status = 'in_use', "usedAt" = NOW() WHERE id = $1`, [token.id]);
        }
        else {
            isValid = false;
        ...
        if (token.status === 'queue') {
            const qRows = await query(`SELECT COUNT(*) AS cnt FROM tokens
                 WHERE "activityId" = $1 AND status = 'queue' AND "createdAt" <= $2`, [token.activityId, token.createdAt]);
...
app.post('/api/tokens/:id/start', async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE tokens SET status = 'in_use', "usedAt" = NOW() WHERE id = $1`, [id]);
...
app.post('/api/staff/:staffId/activity', async (req, res) => {
    try {
        ...
        const staffRows = await query(`SELECT id FROM staff WHERE "userId" = $1 AND "isActive" = true LIMIT 1`, [staffId]);
        if (staffRows.length === 0)
            return res.status(404).json({ error: 'Staff member not found' });
        const activityRows = await query(`SELECT id, name FROM activities WHERE id = $1 AND "isActive" = true LIMIT 1`, [activityId]);
        if (activityRows.length === 0)
            return res.status(404).json({ error: 'Activity not found' });
        const internalStaffId = staffRows[0].id;
        await pool.query(`UPDATE "staffActivities"
             SET "isActive" = false
             WHERE "staffId" = $1 AND "isActive" = true`, [internalStaffId]);
        await pool.query(`INSERT INTO "staffActivities" ("staffId", "activityId", "isActive")
             VALUES ($1, $2, true)`, [internalStaffId, activityId]);
        await pool.query(`UPDATE staff
             SET status = 'active', "updatedAt" = NOW()
             WHERE id = $1`, [internalStaffId]);
        res.json({
...
app.put('/api/activities/:id/capacity-control', async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE activities SET "isCapacityControlOpen" = $1, "updatedAt" = NOW() WHERE id = $2`, [isOpen, id]);
        if (result.rowCount === 0)
            return res.status(404).json({ error: 'Activity not found' });
        if (!isOpen) {
            await pool.query(`UPDATE tokens SET status = 'expired'
                 WHERE "activityId" = $1 AND (status = 'in_use' OR status = 'ready')`, [id]);
            const queueRows = await query(`SELECT id FROM tokens WHERE "activityId" = $1 AND status = 'queue' ORDER BY "createdAt" ASC LIMIT 1`, [id]);
            if (queueRows.length > 0) {
                await pool.query(`UPDATE tokens SET status = 'ready' WHERE id = $1`, [queueRows[0].id]);
            }
            const newActiveRows = await query(`SELECT COUNT(*) AS cnt FROM tokens
                 WHERE "activityId" = $1 AND (status = 'in_use' OR status = 'ready')`, [id]);
            await pool.query(`UPDATE activities SET "currentOccupancy" = $1 WHERE id = $2`, [Number(newActiveRows[0].cnt), id]);
        }
...
app.post('/api/auth/register-staff', async (req, res) => {
    try {
        ...
        const userRows = await query(`INSERT INTO users (email, phone, username, password, role)
             VALUES ($1, $2, $3, $4, $5) RETURNING id`, [email, phone, username, password, role || 'staff']);
        const userId = userRows[0].id;
        const staffRows = await query(`INSERT INTO staff ("userId", "staffId", status, "isActive")
             VALUES ($1, $2, 'active', true) RETURNING id`, [userId, staffId]);
...
app.post('/api/activities', async (req, res) => {
    try {
        ...
        const rows = await query(`INSERT INTO activities (name, description, type, price, capacity, "currentOccupancy",
                                    image, rating, "reviewCount", "safetyRules", "isActive", latitude, longitude)
             VALUES ($1, $2, $3, $4, $5, 0, NULL, $6, $7, $8, true, $9, $10) RETURNING id`, [name, description, type, price, capacity, rating, reviewCount, JSON.stringify(safetyRules), parsedLatitude, parsedLongitude]);
        res.status(201).json({
...
app.post('/api/activities/upload', (req, res, next) => {
    ...
    try {
        ...
        const rows = await query(`INSERT INTO activities (name, description, type, price, capacity, "currentOccupancy",
                                    image, rating, "reviewCount", "safetyRules", "isActive", latitude, longitude)
             VALUES ($1, $2, $3, $4, $5, 0, $6, $7, $8, $9, true, $10, $11) RETURNING id`, [name, description, type, price, capacity, imagePath, rating, reviewCount, JSON.stringify(safetyRules), parsedLatitude, parsedLongitude]);
            ...
            try {
                ...
                if (Array.isArray(staffIds)) {
                    for (const userId of staffIds) {
                        const staffRecords = await query(`SELECT id FROM staff WHERE "userId" = $1 AND "isActive" = true`, [userId]);
                        if (staffRecords.length > 0) {
                            await pool.query(`INSERT INTO "staffActivities" ("staffId", "activityId", "isActive") VALUES ($1, $2, true)`, [staffRecords[0].id, activityId]);
                        }
                    }
                }
            }
...
app.delete('/api/activities/:id', async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE "staffActivities"
             SET "isActive" = false
             WHERE "activityId" = $1 AND "isActive" = true`, [id]);
        const result = await pool.query(`UPDATE activities
...
app.delete('/api/staff/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const staffRows = await query(`SELECT id FROM staff WHERE "userId" = $1 AND "isActive" = true LIMIT 1`, [id]);
        if (staffRows.length === 0)
            return res.status(404).json({ error: 'Staff member not found' });
        const internalStaffId = staffRows[0].id;
        await pool.query(`UPDATE "staffActivities"
...
app.put('/api/users/:id', async (req, res) => {
    try {
        ...
        if (password && password.trim()) {
            result = await pool.query(`UPDATE users SET username = $1, email = $2, phone = $3, password = $4, "updatedAt" = NOW()
                 WHERE id = $5 AND "isActive" = true`, [username, email, phone, password, id]);
        }
        else {
            result = await pool.query(`UPDATE users SET username = $1, email = $2, phone = $3, "updatedAt" = NOW()
                 WHERE id = $4 AND "isActive" = true`, [username, email, phone, id]);
        }
        if (result.rowCount === 0)
...
app.post('/api/payment/process', async (req, res) => {
    try {
        ...
        if (!finalUserId) {
            ...
            const newUser = await query(`INSERT INTO users (email, username, password, role, phone) VALUES ($1, $2, '', 'visitor', $3) RETURNING id`, [`${guestCode}@guest.example.com`, guestCode, phoneNumber]);
        ...
        const paymentRows = await query(`INSERT INTO payments ("transactionId", "userId", "activityId", amount, currency, provider, "phoneNumber", status, "gatewayTransactionId", "processedAt", "gatewayResponse")
             VALUES ($1, $2, $3, $4, 'MWK', $5, $6, 'pending', $7, NOW(), $8) RETURNING id`, [chargeId, finalUserId, activityId || null, amount, provider, phoneNumber, gatewayTransId, gatewayResponse]);
        const paymentId = paymentRows[0].id;
        const tokenCode = 'GT-' + Math.random().toString(36).substr(2, 9).toUpperCase();
        const tokenRows = await query(`INSERT INTO tokens (code, "activityId", "userId", status, "createdAt", "paymentId")
             VALUES ($1, $2, $3, 'pending', NOW(), $4) RETURNING id`, [tokenCode, activityId, finalUserId, paymentId]);
        const response = {
            success: true,
            pending: true,
...
app.post('/api/payment/webhook', async (req, res) => {
    try {
        ...
        if (['successful', 'success'].includes(status) || event === 'charge.successful') {
            await pool.query(`UPDATE payments SET status = 'completed', "gatewayResponse" = "gatewayResponse" || $1, "processedAt" = NOW() WHERE "transactionId" = $2`, ['\nWebhook: ' + JSON.stringify(payload), chargeId]);
            const pendingTokens = await query(`SELECT t.id, t."activityId", a.capacity, a."currentOccupancy"
                 FROM tokens t JOIN activities a ON t."activityId" = a.id
                 WHERE t."paymentId" = $1 AND t.status = 'pending'`, [payment.id]);
            ...
            await pool.query(`UPDATE payments SET status = 'failed', "failedReason" = $1, "gatewayResponse" = "gatewayResponse" || $2, "processedAt" = NOW() WHERE "transactionId" = $3`, [status, '\nWebhook: ' + JSON.stringify(payload), chargeId]);
            ...
            await pool.query(`UPDATE payments SET "gatewayResponse" = "gatewayResponse" || $1 WHERE "transactionId" = $2`, ['\nWebhook: ' + JSON.stringify(payload), chargeId]);
...
app.post('/api/feedback', async (req, res) => {
    try {
        ...
        const rows = await query(`INSERT INTO feedback ("userId", "activityId", rating, comment) VALUES ($1, $2, $3, $4) RETURNING id`, [userId, activityId, rating, comment]);
        await pool.query(`
            UPDATE activities
